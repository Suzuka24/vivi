...
const codiconPaths={
  ...
  viviImage:['M2.5 3.5H13.5V12.5H2.5Z','M3.5 11.5L6.5 8.5L8.5 10.5L10 9L12.5 11.5','M10.5 7A1 1 0 1 0 10.5 5A1 1 0 1 0 10.5 7']
};
function fileIcon(item){
  if(item.directory)return {paths:codiconPaths.folder,color:'var(--vscode-icon-foreground,#c5c5c5)'};
  if(item.supported)return {strokes:codiconPaths.viviImage,color:'var(--vscode-icon-foreground,#c5c5c5)'};
...
function fileIconElement(item){
  ...
  if(icon.paths||icon.strokes){
    ...
    if(icon.strokes){svg.style.fill='none';svg.style.stroke='currentColor';svg.setAttribute('stroke-width','1.15');svg.setAttribute('stroke-linecap','round');svg.setAttribute('stroke-linejoin','round');}
    for(const data of icon.paths||icon.strokes){const path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d',data);svg.append(path);}element.append(svg);
