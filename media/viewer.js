...
function draw() {
  ...
  ctx.setLineDash([]);if(tileMode&&tileViewport())ctx.restore();$('zoom').textContent=`${formatValue(scale*100)}%`;
}
...
canvas.addEventListener('wheel',e=>{e.preventDefault();if(dataset)zoom(e.deltaY<0?1:-1,position(e));},{passive:false});
