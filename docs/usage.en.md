...
| `vivi.keyboardShortcuts` | Action shortcuts such as `shift+p` or `ctrl+shift+h`; macOS Command is `cmd`. Defaults: `=` / `-` zoom, Left/Right switch slices, Up/Down switch Frames, `d` toggles Single/Tile, `p` selects Pointer, `h` selects Hand, `r` selects Rectangle, `c` selects Oval, and `m` measures. An empty string disables an action. Shortcuts work while the image view, but no text field, has focus. |
