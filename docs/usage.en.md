...
**Layout** in the sidebar manages file Frames. A new tab starts in single-Frame mode. Use the display icon to switch to tiled mode, drag Frame rows or use the up, down, top, and bottom toolbar buttons to reorder them, and set optional row or column counts. The eye toggle determines visibility. The adjacent lock toggle determines whether that Frame joins synchronization. Select a Frame before editing it; Up/Down moves selection between visible Frames. Only selected lock participants receive synchronized B&C (including LUT), View (including Zoom), Slice, or Selection changes. A Frame joining an existing lock group adopts the group's enabled settings. In tiled mode, visible locked Frames commit a synchronized display update together.
...
**Adjust** contains Auto methods, Min/Max numeric fields and sliders, Brightness/Contrast sliders, Stretch, LUT, Invert, and a toggleable Curve display. Stretch is a non-destructive display mapping applied after real pixels are normalized by Minimum and Maximum. Curve shows the real-pixel histogram together with the resulting brightness function. Auto/Reset, Histogram, Measure, pixel readouts, and profiles always use real pixels; changing Stretch neither changes those results nor switches back to Linear. Process pixel operations create images with changed values and participate in undo/redo. Threshold displays real pixel values between Minimum and Maximum as white, with values outside as black. Layout's All lock and Unlock only change which Frames participate, leaving B&C, View, Slice, and Selection choices unchanged.
...
| `vivi.mouseShortcuts` | Gestures: `wheel` steps slices (up = previous, down = next), `shift+wheel` zooms at the pointer, `mod+wheel` zooms at image center, `space+click` uses the selected tool in Orthogonal Views, `middle+drag` pans, `alt+right+drag` adjusts brightness and contrast, and `doubleclick` fits. `mod` means Command on macOS and Ctrl on Windows/Linux. Set another chord such as `alt+wheel` or leave empty to disable. |
| `vivi.keyboardShortcuts` | Action shortcuts such as `shift+p` or `ctrl+shift+h`; macOS Command is `cmd`. Defaults: `=` / `-` zoom, Left/Right switch slices, Up/Down switch Frames, `d` toggles Single/Tile, `p` selects Pointer, `h` selects Hand, `r` selects Rectangle, `c` selects Oval, and `m` measures. An empty string disables an action. Shortcuts work in vivi webviews when no text field has focus. Frame Up, Down, Top, and Bottom are extension commands with Shift+Up/Down and Ctrl+Up/Down defaults; change them in Cursor Keyboard Shortcuts, and they remain available from ordinary editor areas. |
