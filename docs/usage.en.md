...
**Adjust** contains Auto methods, Min/Max numeric fields and sliders, Brightness/Contrast sliders, stretch mode, LUT, invert, and a toggleable Curve display. Stretch is a non-destructive virtual pixel transform: the original linear values remain in memory, while display, Adjust, Curve, Auto/Reset, Histogram, Measure, pixel readouts, and profiles use the transformed values. Pixel operations under Process still create images with changed values and participate in undo/redo. Threshold displays transformed values between Min and Max as white, with values outside as black. Layout's All lock and Unlock only change which Frames participate, leaving B&C, LUT, View, Zoom, and Slice choices unchanged.
