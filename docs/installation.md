...
- **大图移动或切片切换较慢：** FITS/TIFF 通常按区域读取；PNG/JPEG 和视频可能需要完整解码。默认像素以原始 dtype 的字节传输，可用 `vivi.losslessCompression` 切换可逆压缩，或为足够大的浮点文件开启 `vivi.lossyCompression`。ZFP 方法需要本环境安装 `zfpy`（已列入 requirements）。`vivi.maxDecodedPixels` 仍保护需要完整解码的格式。
