...
**Adjust** 包含自动调整方法、Min/Max 输入与滑块、Brightness/Contrast 滑块、Stretch、LUT、Invert 和可开关的 Curve 显示曲线。Stretch 是非破坏性的纯显示映射：真实像素先由 Minimum/Maximum 归一化，再由 Stretch 分配窗口内的显示亮度。Curve 同时显示真实像素 histogram 和最终亮度曲线。Auto/Reset、Histogram、Measure、像素读数和剖面始终使用真实像素；改变 Stretch 不会改变这些结果，也不会切回 Linear。Process 中的像素运算会直接生成改变灰度值的新图像，并进入撤销/重做记录。Threshold 可从 Image → Adjust 菜单设置；它把真实像素值位于 Minimum 与 Maximum 之间的像素显示为白色，其余显示为黑色。Layout 的 All lock/Unlock 只勾选或取消所有 Frame 的锁定参与状态，不修改 B&C、View、Slice 或 Selection 的选择。
