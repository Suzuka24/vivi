...
| `vivi.mouseShortcuts` | 鼠标组合手势：默认 `wheel` 切片（上滚向左、下滚向右）、`shift+wheel` 以鼠标缩放、`mod+wheel` 以图像中心缩放、正交视图中 `shift+click` 使用选中工具、`middle+drag` 拖动图像、`alt+right+drag` 调整亮度与对比度、`doubleclick` 适配窗口。`mod` 在 macOS 是 Command，在 Windows/Linux 是 Ctrl。可将动作改为如 `alt+wheel`，或留空禁用。 |
