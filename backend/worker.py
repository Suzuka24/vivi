...
class Session:
    ...
    def handle(self, req):
        ...
        if op == "derive":
            import tifffile
            action = req.get("action")
            supported = {"crop", "flipHorizontal", "flipVertical", "add", "subtract", "multiply", "divide", "normalize", "equalizeHistogram", "histeq", "zMax", "zMean", "zMin", "gaussian", "median", "unsharp", "smooth", "sharpen", "findEdges", "invertPixels", "sqrt", "square", "log", "exp", "abs", "power", "asinh", "sinh", "thresholdBinary", "binaryErode", "binaryDilate", "binaryOpen", "binaryClose", "fftPower", "fftPowerImageJ", "channelRed", "channelGreen", "channelBlue", "to8", "to16", "to32", "toRgb", "resize", "rotateLeft", "rotateRight", "rotate180", "rotate", "mean", "minimum", "maximum", "variance", "findMaxima", "noiseGaussian", "saltPepper", "shadowNorth", "shadowSouth", "shadowEast", "shadowWest", "binaryFillHoles", "binarySkeleton", "binaryWatershed", "removeOutliers", "convolve", "fftBandpass", "stackToRgb", "mergeChannels"}
            ...
            if action == "mergeChannels":
                ...
                selection_original = np.array(result, copy=True)
                if action == "flipHorizontal":
                ...
                elif action in ("equalizeHistogram", "histeq"):
                    data = result.astype(np.float64)
                    ...
                    if action == "smooth":
                        ...
                        if result.dtype != np.uint8 or result.ndim != 2 or not np.isin(np.unique(result), (0, 255)).all():
                            raise ValueError("Skeletonize requires an 8-bit binary image containing only 0 and 255. Use Process > Binary > Make Binary first.")
                        gray=np.mean(data[...,:3],axis=-1) if data.ndim==3 else data
                ...
                elif action in ("invertPixels", "sqrt", "square", "log", "exp", "abs", "power", "asinh", "sinh", "thresholdBinary", "fftPower", "fftPowerImageJ"):
                    data = result.astype(np.float64)
                    if action == "invertPixels": result = np.nanmax(data) + np.nanmin(data) - data
                        ...
                        exponent = finite_number(req.get("value"), 2)
                        if not -100 <= exponent <= 100:
                            raise ValueError("Power exponent must be between -100 and 100")
                        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
                            result = np.power(data, exponent)
                    elif action == "asinh": result = np.arcsinh(data)
                    elif action == "sinh": result = np.sinh(np.clip(data, -50, 50))
                ...
                selection_actions = {"add", "subtract", "multiply", "divide", "normalize", "equalizeHistogram",
                    "gaussian", "median", "unsharp", "smooth", "sharpen", "findEdges", "invertPixels",
                    "sqrt", "square", "log", "exp", "abs", "thresholdBinary", "binaryErode",
                    "binaryDilate", "binaryOpen", "binaryClose", "mean", "minimum", "maximum",
                    "variance", "findMaxima", "noiseGaussian", "saltPepper", "shadowNorth",
                    "shadowSouth", "shadowEast", "shadowWest", "binaryFillHoles", "binarySkeleton",
                    "binaryWatershed", "removeOutliers", "convolve"}
                selected = req.get("selection")
                if action in selection_actions and selected and selected.get("type") in ("roi", "oval", "polygon", "freehand"):
                    if result.shape != selection_original.shape:
                        raise ValueError("This operation cannot preserve the current area selection")
                    mask = selection_mask(selected, box, result.shape[:2])
                    if result.ndim == 3:
                        mask = mask[..., None]
                    result = np.where(mask, result, selection_original)
        ...
        if op == "montage":
            ...
            columns = int(req.get("columns", max(1, round(math.sqrt(slice_count)))))
            if not 1 <= columns <= 32:
            ...
            if isinstance(points, list) and len(points) >= 2:
                ...
                for point in points:
                    if not isinstance(point, list) or len(point) != 2:
                        raise ValueError("Reslice line points must be X,Y pairs")
                    clean.append((finite_number(point[0], 0), finite_number(point[1], 0)))
                samples = []
                for segment, (first, last) in enumerate(zip(clean, clean[1:])):
                    dx, dy = last[0] - first[0], last[1] - first[1]
                    count = max(1, round(math.hypot(dx, dy)))
                    values = [(first[0] + dx * index / count, first[1] + dy * index / count)
                              for index in range(count + 1)]
                    samples.extend(values if segment == 0 else values[1:])
                if any(x < 0 or x > d["width"] - 1 or y < 0 or y > d["height"] - 1 for x, y in samples):
                    raise ValueError("Reslice line must remain inside the image")
                self.stack_limit(len(samples) * (end - start))
                import cv2
                map_x = np.asarray([[point[0] for point in samples]], dtype=np.float32)
                map_y = np.asarray([[point[1] for point in samples]], dtype=np.float32)
                lines = []
                for plane in range(start, end):
                    image = np.ascontiguousarray(self.source.read(d, plane, bounds(d)))
                    line = cv2.remap(image, map_x, map_y, cv2.INTER_LINEAR,
                                     borderMode=cv2.BORDER_REPLICATE)[0]
                    lines.append(line)
                data = np.stack(lines)
                return {"path": self.write_tiff(data, "vivi-reslice-"), "width": len(samples),
                        "height": end - start, "points": clean}
            axis = req.get("axis", "y")
            if axis not in ("x", "y"):
        ...
        if op == "zAxisProfile":
            ...
            values = [self.source.read(d, plane, [x, y, x+1, y+1])[0, 0]
                      for plane in range(start, end)]
        ...
        if op == "pixel":
            ...
            if not (0 <= x < d["width"] and 0 <= y < d["height"]):
                raise ValueError("Pixel out of bounds")
            v = self.source.read(d, frame, [x, y, x + 1, y + 1])[0, 0]
    ...
    def render(self, d, frame, box, req):
        ...
        if req.get('raw'):
            ...
            return result
        if stretch == "histeq" and sample_for_stretch is None:
            sample_step = max(1, math.ceil(max(d["width"], d["height"]) / 512))
            sample_for_stretch = np.asarray(self.source.read(d, frame, bounds(d), sample_step, pyramid=True), dtype=np.float64)
        ...
        if stretch == "log":
            ...
            scaled = (np.power(1000., scaled)-1) / 999
        elif stretch == "sqrt":
            scaled = np.sqrt(scaled)
        elif stretch == "asinh":
            scaled = np.arcsinh(10*scaled) / np.arcsinh(10)
        elif stretch in ("square", "squared"):
            scaled = np.square(scaled)
        elif stretch == "sinh":
            scaled = np.sinh(3*scaled) / np.sinh(3)
        elif stretch == "exp":
            scaled = np.expm1(scaled) / np.expm1(1)
        elif stretch == "abs":
            zero = (0-low)/(high-low)
            left, right = abs(zero), abs(1-zero)
            minimum = 0 if 0 <= zero <= 1 else min(left, right)
            maximum = max(left, right)
            scaled = (np.abs(scaled-zero)-minimum)/(maximum-minimum) if maximum > minimum else np.zeros_like(scaled)
        elif stretch == "histeq":
            values = sample_for_stretch[(sample_for_stretch >= low) & (sample_for_stretch <= high) & np.isfinite(sample_for_stretch)]
            if values.size:
                bins = np.clip(np.floor((values-low)/(high-low)*255), 0, 255).astype(np.uint8)
                counts = np.bincount(bins, minlength=256)
                cumulative = counts.cumsum(dtype=np.float64)
                first = cumulative[np.flatnonzero(counts)[0]]
                cdf = (cumulative-first)/(cumulative[-1]-first) if cumulative[-1] > first else np.zeros(256)
                scaled = cdf[np.clip(np.rint(scaled*255), 0, 255).astype(np.uint8)]
        if req.get("invert"):
            scaled = 1 - scaled
