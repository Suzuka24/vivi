...
- Open PNG, JPEG, TIFF, FITS, and other supported image formats. TIFF stacks, hyperstacks, and multi-HDU FITS files expose their slices or series. **Open As…** shows each series/HDU and accepts rearrange-style axis expressions such as `uv h w` or `c h w`. Import an image folder as a naturally sorted stack, choosing either 2D images only or every plane and optionally requiring a specific `H W`; slices retain their source filenames and plane numbers. Basic AVI/MP4/MOV/MKV previews are also supported when a suitable codec is available.
...
- Keep several files as Frames in one editor tab, switch or tile them, reorder them, and optionally synchronize B&C (including LUT), View (including Zoom), Slice, and Selection across selected Frames. Locked tile updates are committed together. Duplicate titles are numbered automatically; Rename in Layout changes the display title without renaming the source file. **Open in New Tab** creates an independent tab.
- Adjust brightness and contrast, stretch, thresholds, and LUTs; draw and measure regions; inspect pixel values and histograms. The ImageJ-style menu also includes stack conversion, montage, reslice, projection, orthogonal views, profiles, color channels, filters, binary morphology, FFT variants, and image analysis. Pixel operations replace the current Frame and participate in its ten-step undo history; commands with inherently multiple outputs, such as Split Channels, create additional Frames.
- Transfer compressed pixel payloads through a cancellable HTTP stream by default, with the legacy Webview `postMessage` route available in settings. Stack planes remain source ordered, are constructed in the background, and are revealed only after the complete stack is ready. Preview bytes retain the source dtype by default. Lossless transport defaults to Zstandard level 1 with Byte Shuffle; optional lossy encoding is an independent earlier stage for large floating-point previews. Source files and server-side analysis remain unchanged. Large stacks may use substantial client memory.
