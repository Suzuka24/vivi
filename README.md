...
- Cache every stack slice at source resolution after the current slice appears. By default, previews retain the source dtype and optional reversible compression reduces Remote SSH transfer. Optional lossy encoding can reduce transfer further for large floating-point files; it changes preview pixels only. Large stacks may use substantial client memory.
