...
- Transfer a stack through one continuous source-ordered stream, construct every slice in the background, and reveal it only after the complete stack is ready. Preview bytes retain the source dtype by default. Lossless transport defaults to Zstandard level 1 with Byte Shuffle; optional lossy encoding is an independent earlier stage for large floating-point previews. Source files and server-side analysis remain unchanged. Large stacks may use substantial client memory.
