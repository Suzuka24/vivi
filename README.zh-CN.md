...
- 压缩后的像素 payload 默认通过可取消的 HTTP 连续流传输，设置中仍可切换到旧版 Webview `postMessage` 方式。Stack 切片按源顺序在后台构建，全部就绪后一次性显示。预览字节默认保持源 dtype；无损传输默认使用 Zstandard 1级与 Byte Shuffle，可选有损编码作为独立的前置工序。源文件和服务端分析保持不变。大型 stack 可能占用大量本机内存。
